...
import DeathTable from '../../organisms/tables/deaths/Index';
import DeathActivityModal from '../../organisms/modals/deaths/DeathActivityModal';
...
const DeathPatientTemplate = () => {
    const navigate = useNavigate();

    const [isBoardCardModal, setIsBoardCardModal] = useState(false);
        
    const onEditDeath = (params) => {
        return navigate(`/patients/Death/${params.id}/edit`);
    }

    const onClickOpenDeathBoard = () => {
        setIsBoardCardModal(true);
    }

    const onClickCloseDeathBoard = () => {
        setIsBoardCardModal(false);
    }
        ...
        <>
            <div className='patients space-y-8'>
                ...
                <div className="patient-header flex items-center justify-between">
                    <Breadcrumb
                        title={'Death Record Registry'}
                    />
                </div>

                <DeathTable
                    title={'23 Items For 8 Nov 2024'}
                    rows={rows}
                    columns={columns}
                    onEdit={onEditDeath}
                />
            ...
            <DeathActivityModal
                title={'Death Activity'}
                isOpen={isBoardCardModal}
                onClose={onClickCloseDeathBoard}
            />
        </>
    )
}

export default DeathPatientTemplate;
